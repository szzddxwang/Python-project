...
# Sidebar: manual ZIP + original filters
...
# Limit the maximum number of points rendered on the map at once to avoid lag or missing markers
max_points = st.sidebar.slider("Max points on map", 500, 15000, 5000, 500)

# Use session_state to store the active ZIP so we don't re-filter on every keystroke
...
# Filter by ZIP first, then apply other conditions
...
# let listing can be clicked on the map; if none clicked, use the first listing in the current result set
