# tools/pregeocode_all.py — offline geocoding with progress + periodic flush
# 功能：
# - 每批打印进度：[progress] 123/25034 geocoded
# - 定期落盘（默认每 20 批或每 30 秒），中断也不丢进度；下次自动续跑
# - 读取已有缓存 data/geocode_cache.parquet 与种子 data/geocode_seed.csv
# - 支持 Google / Mapbox / Nominatim
#
# 可选环境变量：
#   GOOGLE_API_KEY / MAPBOX_TOKEN
#   GEOCODER_PROVIDER = "google" | "mapbox" | "nominatim"
#   GOOGLE_MAX_WORKERS / MAPBOX_MAX_WORKERS  (默认 12)
#   GOOGLE_QPS / MAPBOX_QPS                  (默认 12)
#   FLUSH_EVERY_BATCHES (默认 20)            → 每 N 批落盘
#   FLUSH_EVERY_SECONDS (默认 30)            → 或每间隔秒数落盘
#
# 运行：
#   python -m tools.pregeocode_all
#
