...
from config import (
    ROLLING_SALES_CSV, GEOCODE_CACHE, GEO_SEED_CSV, FAST_START_STRICT
)
...
def _resolve_sales_path() -> Path:
    ...
    if p_parq.exists(): return p_parq
    if p_feat.exists(): return p_feat
    return csv_path
...
def _load_cache_map() -> Dict[str, Tuple[float, float]]:
    ...
    if Path(GEOCODE_CACHE).exists():
        ...
        m.update({fa: (float(lat), float(lon)) for fa, lat, lon in zip(c["full_address"], c["lat"], c["lon"])})
    if Path(GEO_SEED_CSV).exists():
        ...
        if {"full_address", "lat", "lon"}.issubset(set(s.columns)):
            ...
            m.update({fa: (float(lat), float(lon)) for fa, lat, lon in zip(s["full_address"], s["lat"], s["lon"])})
    return m

@st.cache_data(show_spinner=False)
def attach_geometry(use_address_geocoding: bool = True) -> gpd.GeoDataFrame:
    ...
    if {"LAT", "LON"}.issubset(set(raw.columns)):
        raw_key = (
            ...
            raw["ZIP CODE"].astype(str).str.extract(r"(\d+)", expand=False).str.zfill(5)
        ).apply(_norm_addr)
        key_to_ll = dict(zip(
            raw_key,
            zip(pd.to_numeric(raw["LAT"], errors="coerce"), pd.to_numeric(raw["LON"], errors="coerce"))
        ))
    ...
    if FAST_START_STRICT and missing > 0:
        raise RuntimeError(
            ...
            f"Run tools/pregeocode_all.py to build data/geocode_cache.parquet, "
            f"or add LAT/LON into the CSV. (Instant-start does not call live geocoding.)"
        )

    lst = lst.dropna(subset=["lat", "lon"])
    gdf = gpd.GeoDataFrame(
        lst, geometry=gpd.points_from_xy(lst["lon"], lst["lat"]), crs="EPSG:4326"
    )
    return gdf
