...
def _resolve_sales_path() -> Path:
    ...
    Preference: same basename .parquet -> .feather -> .csv
    """
...
def _load_cache_map() -> Dict[str, Tuple[float, float]]:
    ...
    Priority:
    1) geocode_cache.parquet  (if exists AND parquet engine available)
    2) geocode_cache.csv      (portable, no pyarrow needed)
    3) GEO_SEED_CSV           (extra fallback)
    """
    ...
    # 1) Try parquet (for your own machine)
    if p_parq.exists():
        try:
    ...
    # 2) Fallback to CSV (for portable version)
    ...
    # 3) GEO_SEED_CSV as extra fallback / merge
...
def attach_geometry(use_address_geocoding: bool = True) -> pd.DataFrame:
    ...
    # If raw has LAT/LON columns, prefer them
    if {"LAT", "LON"}.issubset(set(raw.columns)):
        ...
        # Fallback: use pre-geocoded cache files (no live geocoding here)
